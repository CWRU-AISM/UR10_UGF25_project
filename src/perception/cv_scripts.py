...
class TemplateMatching:
    ...
    Use to find specific parts, QR codes regions, fiducial markers
    """
...
class ShapeDetector:
    ...
    Use for part sorting, quality control, pose estimation
    """
...
class FeatureTracker:
    ...
    Use for object tracking, visual odometry, image stitching
    """
...
class ImageSource:
    """Manage image source (camera or folder)"""

    def __init__(self, source_type='folder', folder_path=None):
        ...
        if source_type == 'camera':
            try:
                # Import and initialize Azure Kinect
                from cv_basics import KinectCamera
                self.kinect = KinectCamera(device_id=0)
                print("Azure Kinect initialized successfully")
            except Exception as e:
                print(f"Failed to initialize Azure Kinect: {e}")
                print("Falling back to folder mode")
                self.source_type = 'folder'

        if source_type == 'folder' or self.kinect is None:
            if folder_path and os.path.exists(folder_path):
                # Load all images from folder (case-insensitive)
                ...
                for ext in extensions:
                    ...
                    self.image_list.extend(matches)

                # Remove duplicates and sort
                self.image_list = sorted(list(set(self.image_list)))
                print(f"Found {len(self.image_list)} images in {folder_path}")

                if len(self.image_list) > 0:
                    print(f"First image: {os.path.basename(self.image_list[0])}")
            else:
                print(f"Folder not found: {folder_path}")
                self.image_list = []

    def get_image(self):
        """Get next image from source"""
        if self.source_type == 'camera' and self.kinect:
            color, _ = self.kinect.capture_frame()
            return color
...
def demo_color_detection(image_source):
    """Detect colored objects"""
    print("\nColor Detection Demo")

    detector = ColorDetector()

    # If using folder, process all images
    if image_source.source_type == 'folder':
        print(f"Processing {len(image_source.image_list)} images...")

        for i in range(len(image_source.image_list)):
            ...
            # Draw all detected colors
            result = img.copy()
            for color, contours in results.items():
            ...
            cv2.imshow("Color Detection", cv2.resize(result, (640, 480)))
...
def demo_shape_detection(image_source):
    ...
    if image_source.source_type == 'folder':
        ...
        for i in range(len(image_source.image_list)):
            ...
            for shape in shapes:
                ...
                label = f"{shape['type']} ({shape['vertices']} vtx)"
                ...
                cv2.putText(result, f"C:{shape['circularity']:.2f}", (x, y+h+20),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 0, 0), 1)

            cv2.imshow("Original", cv2.resize(img, (640, 480)))
            cv2.imshow("Shape Detection", cv2.resize(result, (640, 480)))
...
def demo_line_detection(image_source):
    """Detect lines for alignment"""
    print("\nLine Detection Demo")
    ...
    if image_source.source_type == 'folder':
        ...
        for i in range(len(image_source.image_list)):
            ...
            for intersection in intersections:
                ...
                cv2.putText(result, f"{intersection['angle']:.1f}°",
                           (intersection['point'][0]+10, intersection['point'][1]),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 255), 1)

            cv2.imshow("Original", cv2.resize(img, (640, 480)))
            cv2.imshow("Line Detection", cv2.resize(result, (640, 480)))
        ...
        while True:
            ...
            for line in lines:
                cv2.line(result, line['start'], line['end'], (0, 255, 0), 2)
                cv2.circle(result, line['midpoint'], 3, (255, 0, 0), -1)
            ...
            # Display count
            cv2.putText(result, f"Lines: {len(lines)}, Intersections: {len(intersections)}",
...
if __name__ == "__main__":
    ...
    print("1. Use Azure Kinect camera")
    print("2. Load images from folder")

    source_choice = input("\nChoose image source (1 or 2): ").strip()

    image_source = None

    if source_choice == "1":
        # Try to use camera
        image_source = ImageSource(source_type='camera')
        ...
        image_source = ImageSource(source_type='folder', folder_path=folder_path)

        if len(image_source.image_list) == 0:
            print("\nError: No images found in the specified folder!")
            print("Supported formats: JPG, JPEG, PNG, BMP (case-insensitive)")
            exit(1)
    else:
    ...
    try:
        ...
        while True:
            ...
            if choice == "1":
                demo_color_detection(image_source)
            elif choice == "2":
                demo_shape_detection(image_source)
            elif choice == "3":
                demo_line_detection(image_source)
            elif choice == "0":
                break
            else:
